...
const addStudent = async (req: Request, res: Response) => {
    ...
    let studentDescription = req.body.description;
    let studentTechonologie = req.body.technologie;
    let studentDate = req.body.date;

    
    let s = await Student.findOne({name: studentName});  
    if(!s) { 
        let student = new Student({ "name": studentName, "description": studentDescription, "techonologie": studentTechonologie, "date": studentDate });
    ...
    await Subject.updateOne({"name": subjectName}, {$addToSet: {students: s?._id}}).then(data => { 
        if (data.nModified == 1) { 
            ...
            res.status(409).json('Vaccine already exists!!!') 
    } }).catch((err) => { 
